		...
		public override void CalculateLayoutInputHorizontal()
		{
			...
			else if (fittingMethod == FittingMethod.Spacing)
			{
				if (fixedSide == FixedSide.Columns && columns > 1)
				{
					...
					/*                    if (result < minSpacing || maxSpacing < minSpacing)
										{
											result = minSpacing;
										}
										else if (result > maxSpacing)
										{
											int horizontalAdjustment = Mathf.FloorToInt();
											padding.left += horizontalAdjustment;
											padding.right += horizontalAdjustment;
											result = maxSpacing;
										}*/
			...
			if (fitToContent == FitToContent.Width || fitToContent == FitToContent.Both)
			{
				...
				{
					this.SmartWarning("Can't fit to content's width. Wrong anchor's positions!");
			...
			if (fitToContent == FitToContent.Height || fitToContent == FitToContent.Both)
			{
				...
				{
					this.SmartWarning("Can't fit to content's height. Wrong anchor's positions!");
